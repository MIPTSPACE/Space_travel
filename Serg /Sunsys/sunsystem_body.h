#ifndef SUNSYSTEM_BODY
#define SUNSYSTEM_BODY
...
#define G           6671919999e-20
#define PI          3.1415926535
#define ONE_DEG_RAD 0.0174532925
...
public:
    ...
    //це я додав собі для зручності
    vector <planet>* getSomePlanets(){return &plmas;}

    //це теж. це фунцкія яка рухає кульки
    //(у мене вони рух. по колу, я думаю це не важливо,
    //адже хоч еліптичну орбіту я зроблю, хоч круглу,
    //це прийдеться переробити
    //ти маєш дописати замість цього якусь фізику))
    void makeStep(double len){
        for (int i = 0; i < plmas.size(); i++){
            double alpha = acos(plmas[i].posx/plmas[i].rad);
            if (plmas[i].posy < 0) alpha = 2*PI - alpha;
            alpha += len/(2*PI*plmas[i].rad);
            plmas[i].posx = cos(alpha)*plmas[i].rad;
            plmas[i].posy = sin(alpha)*plmas[i].rad;
        }
    }
...
//functions
    ...
    //////////PEREROBYTY////////////////////////
    void addplanet(double rad, double v){
        planet n;
        n.posx=rad;
        n.posy=0;
        n.rad=rad;
